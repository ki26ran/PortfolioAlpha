...
def _get_broker_api(strat_cfg):
    ...
    if not live:
        return None
    bn = strat_cfg.get("broker_name", "xyz")
    bu = strat_cfg.get("broker_username", "pqr")
    cache_key = f"{bn}|{bu}"
    if cache_key in _broker_api_cache:
        return _broker_api_cache[cache_key]
    try:
        sys.path.insert(0, ROOT)
        sys.path.insert(1, os.path.join(BASE, "broker"))
        from broker.api import setup_api
        api = setup_api(bn, bu)
...
def _place_entry_swing(api, sym, qty, direction, strat_id, entry_price, sl, target):
    if api is None:
        return None
    try:
        ...
        from broker.api import place_live_order
        oid = place_live_order(
            f"{sym}-EQ", side, qty, remarks,
...
def _place_exit_swing(api, sym, qty, direction, reason, strat_id, exit_price):
    if api is None:
        return None
    try:
...
def _fetch_intraday_cached(ticker, interval, start, end):
    """Fetch intraday data from DuckDB cache. Fallback to live provider on miss."""
    try:
        cache = get_cache()
        ...
        print(f"  [WARN] Cache intraday failed for {ticker}: {e}")
    return None
...
def _phase_entry():
    ...
    for strat in _get_strats():
        ...
        for sel in selections[:max_pos]:
            ...
            try:
                ...
                rows.append({"symbol": sym, "direction": direction, "entry": round(entry_price, 2),
                ...
                positions.append({"symbol": sym, "direction": direction, "entry_price": round(entry_price, 2),
    ...
    save_csv(SIGNALS_FILE, ["symbol", "direction", "entry", "sl", "target", "qty", "entry_time", "strategy", "scanned_at"], all_rows)
...
def _send_exit_alert(pos, exit_price, exit_time, reason):
    ...
    try:
        ...
        if not isinstance(history, list):
            history = []
        hist_entry = {k: pos.get(k) for k in ["symbol", "direction", "entry_price", "qty", "initial_sl", "target",
    ...
    while _now() < close_time:
        ...
        if _unfilled_signals and not after_entry_window:
                ...
                for sel, sid, entry_mode, capital, retry_n in _unfilled_signals:
                    ...
                    changed = True
                    print(f"    ENTERED {sym} {direction} @ Rs.{ep:,.2f} qty={qty}")
                    if is_configured():
                        sname = _strategy_names.get(sid, sid)
