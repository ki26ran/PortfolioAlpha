...
def show():
    ...
    if run_btn:
        ...
        results = {}
        with st.spinner(f"Running backtests ({len(selected_ids)} strategies)..."):
            for sid in selected_ids:
                ...
                if strat is None:
                    ...
                    continue
                try:
                    # Use per-strategy universe, fallback to global selection
                    ...
                    st.info(f"{strat.name}: {len(trades)} trades")
